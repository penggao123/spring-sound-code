...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	/** BeanFactoryPostProcessors to apply on refresh xx */
     ...
     * 它除了支持ResourceLoader支持的前缀外，还额外支持“classpath*:”用于加载所有匹配的类路径Resource，
	 */
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			//1:准备刷新上下文环境  生命早期的监听器和事件，不需要手动调用publishEvent
//			做了一些刷新前的准备工作，和主流程关系不大，主要是保存了容器的启动时间，启动标志等
			prepareRefresh();

			//2:获取告诉子类初始化Bean工厂  不同工厂不同实现
//			该方法可以简单的认为，就是把beanFactory取出来而已。XML模式下会在这里读取BeanDefinition
			...
			try {
				...
				//解析配置类，通过ConfigurationClassPostProcessor类扫描成Bean定义的
				//此方法只实例化实现了Bean工厂后置处理器接口的类，进行getBean()
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//注册了一个事件监听器探测器后置处理器接口， ApplicationListenerDetector解析接口方式的监听器
	...
	/**
	 *  xml情况下 beanDefinition已加载，提供注册bean的后置处理器
	...
	protected void registerListeners() {
		...
		//获取bean定义中的监听器对象  防止懒加载的监听器漏网
		// 接口方式的监听器就是在这里注册到多波器里面的 除了这个地方，还有一个地方
